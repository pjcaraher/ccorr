...
def user_for_id(userId) :
	...
	try :
		...
		db.session.rollback()
		print('Exception fetching user id [%s] %s' % (str(userId), str(ex)))
		user = None
...
def save_job_map(mapName, s3Key, type, job) :
	...
	try :
		...
		db.session.rollback()
		WarningMessage = "Unable to create new Job " + str(ex)
...
def update_job():
	...
	try :
		...
		db.session.rollback()
		WarningMessage = "Unable to update Job " + str(ex)
...
#         db.session.rollback()
#     	print(dir(ex))
#     	WarningMessage = "Unable to create update Vendor " + str(vendor.name) + " " + str(ex)
# 
...
def update_user():
	...
	try :
		...
		db.session.rollback()
		print(dir(ex))
		WarningMessage = f"Unable to update User {str(user.email)} {str(ex)}"
...
#    	else :
	...
	try :
		...
		db.session.rollback()
		print(dir(ex))
		WarningMessage = "Unable to create new User " + str(newUser.email) + " " + str(ex)
...
def resend_newuser_email():
	...
	try :
		...
		db.session.rollback()
		print(dir(ex))
		WarningMessage = "Unable to resend invite to User " + str(newUser.email) + " " + str(ex)
...
def hide_user():
	...
	if userToHide :
		try :
			...
			db.session.rollback()
			print(dir(ex))
			WarningMessage = "Unable to remove User " + str(userToHide.email) + " " + str(ex)
...
def hide_vendor():
	...
	if vendorToHide :
		try :
			...
			db.session.rollback()
			print(dir(ex))
			WarningMessage = "Unable to remove Vendor " + str(vendorToHide.email) + " " + str(ex)
